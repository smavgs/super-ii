...
const scriptSourceSet = new Set(scriptSources);
const connectSourceSet = new Set(connectSources);
assert.ok(!scriptSourceSet.has("'unsafe-inline'"), 'normal script-src must not allow unsafe-inline');
assert.ok(!scriptSourceSet.has('https:'), 'normal script-src must not allow every HTTPS origin');
assert.ok(scriptSourceSet.has("'wasm-unsafe-eval'"), 'browser inference requires bounded WebAssembly compilation');
assert.ok(scriptSourceSet.has('https://clerk.superii.site'), 'Clerk frontend API must remain available');
assert.ok(scriptSourceSet.has('https://challenges.cloudflare.com'), 'Cloudflare challenges must remain available');
assert.ok(!connectSourceSet.has('https:'), 'connect-src must not allow every HTTPS origin');
assert.ok(!connectSourceSet.has('wss:'), 'connect-src must not allow every secure WebSocket origin');
