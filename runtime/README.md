...
- A publication-time verified tokenizer gate for every applicable text model. Safe native loaders compare plain, multilingual, emoji, and special-token vectors against a normalized content-addressed pack. GGUF metadata is converted into portable `tokenizer.json`, then every token ID is checked against the pinned `llama-tokenize` oracle before publication. Published packs support exact revision-addressed encode, decode, artifact retrieval, REST, MCP, SDK, CLI, and a browser fallback without executing repository Python or loading model weights.
