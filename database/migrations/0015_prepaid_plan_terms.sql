...
set billing_term = '30_days'
where billing_term is null;

alter table app.payment_orders
  alter column billing_term set default '30_days',
  alter column billing_term set not null;

alter table app.payment_orders
  drop constraint if exists payment_orders_billing_term;
alter table app.payment_orders
  add constraint payment_orders_billing_term check (
    billing_term in ('30_days', '12_months')
  );

drop function if exists app.create_or_reuse_payment_order(uuid, uuid, text, integer, integer);
...
create or replace function app.create_or_reuse_payment_order(
  ...
  p_billing_term text default '30_days'
)
...
begin
  if p_plan_id not in ('pro', 'team')
    ...
    or p_price_amount_cents < 1
    or p_billing_term not in ('30_days', '12_months') then
  ...
  expected_price := case p_billing_term
    when '30_days' then unit_price * p_seat_count
    when '12_months' then round(
      (unit_price * p_seat_count * 12)::numeric * 0.80
    )::integer
  end;
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    p_profile_id::text || ':' || coalesce(p_organization_id::text, 'personal')
      || ':' || p_plan_id || ':' || p_seat_count::text || ':' || p_billing_term,
    0
  ));
  select * into target
  ...
  where profile_id = p_profile_id
    ...
    and billing_term = p_billing_term
  ...
  insert into app.payment_orders (
    ...
    p_profile_id, p_organization_id, p_plan_id, p_seat_count, p_billing_term,
...
begin
  ...
  if effective_status = 'finished' and target.status <> 'finished' then
    entitlement_interval := case target.billing_term
      when '12_months' then interval '12 months'
      else interval '30 days'
    end;
    entitlement_label := case target.billing_term
    ...
    select greatest(now(), coalesce(max(s.current_period_end), now())) + entitlement_interval
    into period_end
    ...
    values (
      ...
      format('USDC payment confirmed. Your plan is active for the next %s.', entitlement_label),
      '/account',
      jsonb_build_object(
        'order_id', p_order_id,
        ...
        'billing_term', target.billing_term
      )
    );
