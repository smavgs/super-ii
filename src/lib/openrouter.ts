...
function groundedSearchMessages(
  ...
  if (!latest) return providerMessages(messages, skillContext, grounding);
  const searchData = {
  ...
  return providerMessages(groundedMessages, skillContext, grounding);
}
...
export function openRouterSearchAnswerRequest(
  ...
  return {
    model: OPENROUTER_MODEL,
    messages: groundedSearchMessages(messages, toolCall, sources, skillContext, grounding),
    max_completion_tokens: OPENROUTER_MAX_COMPLETION_TOKENS,
    reasoning: OPENROUTER_REASONING,
    temperature: 0.25,
  };
