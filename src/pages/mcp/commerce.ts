  ...
  if (request.method !== 'OPTIONS') {
    ...
    if (rate !== 'allowed') {
      return Response.json({
        error: rate === 'limited' ? 'Commerce MCP rate limit reached' : 'Commerce MCP safety service unavailable',
      }, { status: rate === 'limited' ? 429 : 503 });
    }
  }
  const origin = new URL(request.url).origin;
  return createSuperiiCommerceMcpHandler(locals, origin, request).fetch(request);
};
